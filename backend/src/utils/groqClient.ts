...
export class GroqClient {
  ...
  constructor() {
    ...
    if (!apiKey) {
      console.warn("[GroqClient] GROQ_API_KEY is not set.");
    }
  ...
  private async callWithRetry<T>(fn: () => Promise<T>, retries = 3, delayMs = 1000): Promise<T> {
    try {
      ...
      if ((isRateLimit || isServerErr) && retries > 0) {
        console.warn(`[GroqClient] API call failed. Retrying in ${delayMs}ms. Retries left: ${retries}. Reason: ${error.message}`);
